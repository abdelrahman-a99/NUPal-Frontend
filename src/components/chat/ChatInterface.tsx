...
// ─── Eye that tracks the mouse cursor (hidden – add <EyeTracker /> to restore) ──
...
export default function ChatInterface({
  ...
  // Sync input when chat or draft changes
  useEffect(() => {
  ...
  // Scroll to bottom whenever messages change or chat switches
  useEffect(() => {
  ...
  const renderSkeleton = () => (
    <div className="mx-auto max-w-3xl space-y-4 py-4">
      ...
      <div className="flex w-full justify-start mb-4">
        <Skeleton className="h-12 w-[50%] max-w-[70%] rounded-2xl" />
      </div>
    </div>
  );
        ...
        >
          <div className="flex items-center">
            ...
            <Button
              ...
              className={`group absolute right-3 flex h-10 w-10 items-center justify-center rounded-lg bg-transparent transition-all duration-300 hover:bg-blue-50 disabled:opacity-50 ${
                !inputMessage.trim() || isLoading ? 'pointer-events-none' : 'cursor-pointer'
              }`}
            >
              <svg
                height="20"
                ...
                <path
                  ...
                  className={`transition-all duration-300 ${
                    inputMessage.trim() && !isLoading
                      ? 'group-hover:stroke-blue-500 group-hover:fill-blue-500'
                      : ''
                  }`}
                />
              </svg>
  ...
  return (
    <div className="flex h-full flex-1 flex-col bg-white dark:bg-slate-900 overflow-hidden">
      {!chatId ? (
        /* No chat selected: centred welcome + input */
