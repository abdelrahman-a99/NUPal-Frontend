...
function Toggle({ checked, onChange, id }: { checked: boolean; onChange: (v: boolean) => void; id: string }) {
    return (
        <button
            ...
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${checked ? 'bg-blue-500' : 'bg-slate-200'}`}
        >
            <span
    ...
    return (
        <div className="flex items-center justify-between px-6 py-4">
            <div className="mr-4">
                ...
                {description && <p className="text-xs text-slate-400 mt-0.5 leading-snug max-w-xs">{description}</p>}
            </div>
    ...
    return (
        <button
            ...
            className={`flex flex-col items-center gap-1.5 px-4 py-3 rounded-xl border text-sm font-semibold transition-all ${
                active ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-500 hover:border-slate-300 hover:bg-slate-50'
            }`}
        >
            <Icon size={18} />
            {label}
...
export default function SettingsPage() {
    ...
    const update = <K extends keyof SettingsState>(key: K, value: SettingsState[K]) => {
        setSettings(prev => {
            ...
            // Immediately apply runtime classes
            if (key === 'autoSaveDrafts') {
                // Handled in scheduling pages
            }
            if (key === 'theme') {
            ...
            <div className="max-w-3xl mx-auto px-6 py-8">
                ...
                <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 mb-6 flex items-center gap-5 shadow-xl">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-slate-400 text-sm mt-0.5 truncate">{userEmail}</p>
                ...
                <SectionCard title="Appearance" description="Customize how NUPAL looks for you" icon={Sun}>
                    <div className="px-6 py-5">
                        <p className="text-sm font-semibold text-slate-800 mb-3">Website Mode</p>
                        ...
                        <p className="text-xs text-slate-400 mt-2">Note: Dark mode UI coming in a future release.</p>
                    </div>
                ...
                <SectionCard title="Language & Region" description="Set your display language" icon={Globe}>
                    <div className="px-6 py-5">
                        <p className="text-sm font-semibold text-slate-800 mb-3">Display Language</p>
                        <div className="flex gap-3">
                            {([
                        ...
                        <p className="text-xs text-slate-400 mt-2">Arabic (RTL) support coming soon.</p>
                    </div>
                </SectionCard>
                ...
                <SectionCard title="Data & Privacy" description="Manage your data and exports" icon={Eye}>
                    <SettingRow label="Download Academic Data" description="Export a copy of your full academic transcript and registration data as a PDF.">
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-slate-50 hover:bg-slate-100 text-slate-700 text-sm font-bold rounded-lg border border-slate-200 transition-colors"
                        >
                ...
                <div className="border border-red-200 rounded-2xl overflow-hidden">
                    <button
                        ...
                        className="w-full flex items-center justify-between px-6 py-4 bg-red-50 hover:bg-red-100/50 transition-colors text-left"
                    >
                        <div className="flex items-center gap-3">
                    ...
                    {showDangerZone && (
                        <div className="px-6 py-5 bg-white border-t border-red-100">
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-red-300 text-red-600 text-sm font-bold hover:bg-red-50 transition-colors"
                            >
                                <LogOut size={15} />
                                Log Out
                            </button>
